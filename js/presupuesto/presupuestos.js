...
function obtener_datos(id) {
	...
	if (month < 10) {
		var actual = `${year}-0${month}-${day}`;
	} else {
		var actual = `${year}-${month}-${day}`;
	}
	if (fecha_presupuesto == actual) {
		$("#myModalLabel1").text('Editar presupuesto');
		...
		$("#myModalLabel1").text(`No se pueden editar registros anteriores a la fecha: ` + actual);
		$("#cuerpo").hide();
